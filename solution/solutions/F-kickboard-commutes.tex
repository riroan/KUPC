...
\begin{frame}{\textbf{F}. 킥보드로 등교하기}
    \begin{itemize}
        ...
        \item 이 때, 킥보드의 용량 $L$에 대해 모든 경우를 순차적으로 탐색하면, 특정 용량일 때 통학 가능 여부를 판단하는 로직 $O(N)$을 최대 $L$번 수행하므로 $O(NL)$로 시간초과를 받습니다.
    \end{itemize}
\end{frame}

\begin{frame}{\textbf{F}. 킥보드로 등교하기}
    \begin{itemize}
        \item 따라서 킥보드 용량과 충전 횟수 관계가 서로 단조증감함수 성질을 지닌다는 점을 이용해 이분 탐색으로 $O(NlogL)$로 해결해야 합니다.
